...
const SeededRecentsBanner ="This recent tab order was generated by Chrome and may be inaccurate. It will improve as you switch tabs with QuicKey installed.";
		...
		context)
	{
		...
		this.state = {
			...
			newSettingsAvailable: false,
				// default to hidden and let countBannerDisplay() turn it on
				// once we've read the count, so the banner can't flash on for
				// the profiles that should never see it at all
			bannerVisible: false
		};
	}
...
//						.then(() => this.setState({ selected }));
	...
	async showWindow({
		...
		activeTab})
	{
			// only count this as a banner display if the window was hidden,
			// so that showing an already visible window doesn't burn one of
			// the few times we get to show it.  componentDidMount() counts
		...
		event) =>
	{
		if (event.key == this.mruModifier) {
			...
			this.gotMRUKey = false;
				...
				// Chrome seems to have changed its behavior, and we need to prevent
				// default on esc keyUp as well to stop the popup from closing
			event.preventDefault();
		}
	};


		// the banner explains that a newly seeded recents list is only a guess
		// at the right order.  it's shown the first few times the menu or popup
		// is opened after the install that seeded it, and then hides itself,
		// since by then the list is mostly built from tab switches we've seen.
	async countBannerDisplay()
	{
		const {seededRecentsBannerCount: count} = await storage.get();
		const bannerVisible = count < k.MaxSeededRecentsBannerDisplays;

		this.setState({ bannerVisible });

		if (bannerVisible) {
				// re-read the count inside set(), rather than incrementing the
				// one above, since the menu and the popup window can both be
				// open at once
		...
		// closing the banner is permanent, so jump the count to the limit
	onBannerClose = async () =>
	{
