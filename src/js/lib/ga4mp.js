// copied from https://github.com/analytics-debugger/ga4mp/blob/main/dist/ga4mp.esm.js
...
const sendRequest = (endpoint, payload, mode = 'browser', opts = {}) => {
	...
	if (mode === 'browser') {
			// EDIT: check if we can use sendBeacon() or fetch(), since the former
			// isn't included in background scripts/service workers
...
const clientHints = (mode) => {
		// EDIT: check for navigator as a global instead of window, since window
		// is not defined in background scripts, but we still want to treat them
		// as browser mode
	if (mode === 'node' || typeof(navigator) !== 'object' || !navigator) {
 ...
 */
const pageDetails = () => {
		// EDIT: the document and screen objects aren't available in background
		// scripts, but language is
	const details = {
		language: (navigator?.language || navigator?.browserLanguage || '').toLowerCase(),
	};

	if (globalThis.document && globalThis.screen) {
		details.page_location = document.location.href;
		details.page_title = document.title;
		details.screen_resolution = `${screen.width}x${screen.height}`;

		if (document.referrer) {
			details.page_referrer = document.referrer;
		}
	}
...
const ga4mp = function (measurement_ids, config = {}) {
	...
	const setClientId = (value) => {
		internalModel.payloadData.client_id = sanitizeValue(value, 40);
	};
